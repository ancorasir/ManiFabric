...
# sys.path.append("/home/hlab/FabricMani/FabricMani/FabricMani/real_robot/utils/")
# from euler import quat2euler, euler2quat
# from rs_utils import compute_transformation_matrix

from utils.euler import quat2euler, euler2quat
from utils.rs_utils import compute_transformation_matrix
...
class Robot:
    ...
    def __init__(self, config, **kwargs):
        ...
        # self.client_set_mode = actionlib.SimpleActionClient(f"/{self.robot_name}/ur_hardware_interface/set_mode", ur_dashboard_msgs.msg.SetModeAction)
    ...
    def get_ee_pose(self):
        target_frame = '{}/tool0_controller'.format(self.robot_name)
        source_frame = '{}/base'.format(self.robot_name)
        try:
...
if __name__ == "__main__":
    ...
    # NOTE: if run codes after "move_to_init_pose", we need sleep some times to wait for robot to finish the task
    ...
    # actions = robot_right._collect_trajectory(current_picker_position, current_picker_position+np.array([[0., 0., 0.1], [0., 0., 0.1]]))
    # traj_left = robot_left.prepare_traj(actions[:,:3], robot_left.dt)

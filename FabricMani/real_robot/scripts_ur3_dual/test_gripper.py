...
class Robotiq2FGripperURCapBridge:
    ...
    def move(self, position, speed, force):
        ...
        var_dict = dict([(self.POS, clip_pos), (self.SPE, clip_spe), (self.FOR, clip_for)]) #, (self.GTO, 1) in []
    ...
    def move_and_wait_for_pos(self, position, speed, force):
        ...
        # wait until the gripper goes to the requested position OR its position is unchanged for a timeout
        timeout = 0.5 # seconds
        stable_start_time = time.time()
        last_position = int(self.get_current_position())
        while int(self.get_current_position()) != cmd_pos:
            cur_position = int(self.get_current_position())
            # print(f"cur_position: {cur_position}")
            if cur_position != last_position:
                stable_start_time = time.time()
            last_position = cur_position
            if time.time() - stable_start_time > timeout:
                break
            time.sleep(0.001)
